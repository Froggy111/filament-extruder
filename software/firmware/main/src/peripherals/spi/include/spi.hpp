...
struct Config {
    ...
    DMA_Channel_TypeDef* rx_dma_channel = nullptr;
    DMA_Channel_TypeDef* tx_dma_channel = nullptr;
    uint32_t rx_dma_request = 0;
    uint32_t tx_dma_request = 0;
...
void start_dma_receive(Config& spi, uint8_t* buffer, uint16_t size);
bool is_dma_receive_done(Config& spi);
void end_dma_receive(Config& spi);
}  // namespace spi
