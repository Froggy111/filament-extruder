...
void uart::init(uint32_t baud_rate) {
    ...
    // max priority that can call FreeRTOS fromISR functions
...
void uart::transmit_raw(uint8_t* data, uint16_t len) {
    if (transmission_completed) {
        transmission_completed = false;
        memcpy((void*)&tx_buffer[0], data, len);
        HAL_UART_Transmit_DMA(&uart_handle, (uint8_t*)tx_buffer, len);
    }
}

void uart::transmit_raw_blocking(uint8_t* data, uint16_t len) {
